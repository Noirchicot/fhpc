...
export const FH_UNDERIVED_FR = {
  ...
  "underived.fh.skillpool-bound-grows": (d) =>
    ...
    `« ${d.classId} », captifs de : ${d.from || "(liste non déclarée)"}. L'aptitude NOMME une liste, donc ` +
    "elle contraint (canon §B.1quater). Le bound n'est donc pas figé à la création — mais il ne grandit " +
    "JAMAIS par un point d'échelle, seulement quand une aptitude nomme une liste, à quelque niveau qu'elle tombe.",
...
export const FH_UNDERIVED_EN = {
  ...
  "underived.fh.skillpool-bound-grows": (d) =>
    ...
    "JAMAIS par un point d'échelle, seulement quand une aptitude nomme une liste, à quelque niveau qu'elle tombe.",
  "underived.fh.skillpool-bound-grows": (d) =>
    `"${d.feature}" (level ${d.level}) adds ${d.points} bound skill point(s) to "${d.classId}", captive to: ` +
    `${d.from || "(list not declared)"}. The feature NAMES a list, so it constrains (canon §B.1quater). Bound ` +
    "is therefore not frozen at creation — but it never grows from a ladder point, only when a feature names " +
    "a list, at whatever level it fires.",
  "underived.fh.skillpool-bound-not-in-pool": (d) =>
