...
export async function bootEngine({ root = "../.." } = {}) {
  const { createLayers } = await import("../../src/layers/index.mjs?v=690");
  const { createBuild } = await import("../../src/build/index.mjs?v=690");
  const { createFhDestinyStat } = await import("../../src/modules/fh/destiny-stat.mjs?v=690");
  const { createFhSkillPoolStat } = await import("../../src/modules/fh/skill-pool.mjs?v=690");
  ...
  const { createFhSpeciesTraits } = await import("../../src/modules/fh/species-traits.mjs?v=690");
