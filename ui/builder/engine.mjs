...
export async function bootEngine({ root = "../.." } = {}) {
  const { createLayers } = await import("../../src/layers/index.mjs?v=374");
  const { createBuild } = await import("../../src/build/index.mjs?v=374");
  const { createFhDestinyStat } = await import("../../src/modules/fh/destiny-stat.mjs?v=374");
  const { createFhSkillPoolStat } = await import("../../src/modules/fh/skill-pool.mjs?v=374");
