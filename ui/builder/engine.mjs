...
export async function bootEngine({ root = "../.." } = {}) {
  const { createLayers } = await import("../../src/layers/index.mjs?v=768");
  const { createBuild } = await import("../../src/build/index.mjs?v=768");
  const { createFhDestinyStat } = await import("../../src/modules/fh/destiny-stat.mjs?v=768");
  const { createFhSkillPoolStat } = await import("../../src/modules/fh/skill-pool.mjs?v=768");
  ...
  const { createFhSpeciesTraits } = await import("../../src/modules/fh/species-traits.mjs?v=768");
