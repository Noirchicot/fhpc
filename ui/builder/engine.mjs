...
export async function bootEngine({ root = "../.." } = {}) {
  const { createLayers } = await import("../../src/layers/index.mjs?v=279");
  const { createBuild } = await import("../../src/build/index.mjs?v=279");
  const { createFhDestinyStat } = await import("../../src/modules/fh/destiny-stat.mjs?v=279");
  const { createFhSkillPoolStat } = await import("../../src/modules/fh/skill-pool.mjs?v=279");
