...
import { bootEngine, loadExampleDocument, loadDocSchema } from "./engine.mjs?v=847";
import { swapContent, keepInView, watchSnap, mountChevrons } from "./socle.mjs?v=847";
import { mountPopup } from "./popup.mjs?v=847";
import { renderLorePanel } from "./lore.mjs?v=847";
import { nomDeFichier, renderReviewStep, reviewValidate } from "./review-step.mjs?v=847";
...
import { estConfirme, refusDuDone, etatDeLEtape, etapeAchevee, itemsDeLEtape, refsMortsDeLEtape, ETAT } from "./parcours.mjs?v=847";
import { STEPS, ceinture, cransAlignes, manqueDuCran } from "./etapes.mjs?v=847";
/* 🔴 LOT 201 — UN ÉCRAN VIDE SE NOMME. `ecran-mort.mjs` couvre l'écran qui
   ...
   rendus. */
import { nommerLeVide } from "./ecran-vide.mjs?v=847";
import { renderGuideSpecifique, renderItem as renderItemDalle, renderBilan, renderGuideGeneral, motDe } from "./parcours-ecrans.mjs?v=847";
import {
...
import { lirePersonnage, ecrirePersonnage, oublierPersonnage } from "./memoire.mjs?v=847";
import { lireLeFichier } from "./ouvrir.mjs?v=847";
...
import { vueDoubleVoulue, setVueDoubleVoulue } from "./vue.mjs?v=847";
...
import { planAt, planSlots } from "./carnet.mjs?v=847";
import { renderChoixGlisses } from "./glisser.mjs?v=847";
import { renderConceptStep } from "./concept-step.mjs?v=847";
import { renderUniverseStep, currentStack, fhRefChoices, FH_LAYER_IDS, sauvegarderPuisEteindre, NOM_DE_LA_VERSION_FH,
  creerUnPersonnage, popupDuJeu, NOM_DU_PERSONNAGE_NEUF } from "./universe-step.mjs?v=847";
...
import { couchesApresLeGeste, gestesDAlignement, manifesteDeLaPile } from "./layers-ecran.mjs?v=847";
...
import { motDeLEcranMort, MOT_CRAN_NON_MONTE, motDesChoixNonResolus } from "./ecran-mort.mjs?v=847";
...
import { motDuChoix, motDUnRecordAbsent } from "./mot-du-choix.mjs?v=847";
import { renderSkillsStep, skillsValidate, motDuVerrou, skillsCheminsDeReset, skillsReinitialiserEcran, skillsFermerAjout, skillsAnnulerAjout } from "./skills-step.mjs?v=847";
import {
  catalogueCursor, catalogueValidate, renderCatalogueRail, renderCatalogueCards
} from "./catalogue.mjs?v=847";
import { CLASS_CATALOGUE, renderClassCardBody, renderClassChoices, classPalier2 } from "./class-step.mjs?v=847";
import { SPECIES_CATALOGUE, renderSpeciesCardBody, renderSpeciesChoices, speciesPalier2 } from "./species-step.mjs?v=847";
...
import { BACKGROUND_CATALOGUE, renderBackgroundCardBody, renderBackgroundChoices, backgroundPalier2, inheritanceMontee } from "./background-step.mjs?v=847";
...
import { chargerLaFicheDeSecours } from "./fiche-secours.mjs?v=847";
import { renderInheritanceStep, inheritanceValidate, renderBoostGlisse,
  ...
  featSousLabel, featInfo } from "./inheritance-step.mjs?v=847";
import {
...
import { ABILITY_KEYS } from "../../src/build/index.mjs?v=847";
import {
  renderDestinyStep, renderDestinyFinal, destinyValidate, currentArcanaId, drawArcana,
  DESTINY_ARCANA_PATH, arcanaNumeral
} from "./destiny-step.mjs?v=847";
import { renderCeremonie, DUREES as DESTINY_DUREES } from "./destiny-ceremonie.mjs?v=847";
import { renderEquipmentStep, equipmentValidate, currentCurrency, nextGearIndex, currentGearLines,
         ...
         boitesDehors, scinderLaLigne, retirerLaLigne, accorderLEquipe, appliquerLeButin, verserLeKit } from "./equipment-step.mjs?v=847";
...
import { CASES_DU_SAC, poserLesDalles } from "./sac-ecran.mjs?v=847";
/* ⭐ MÊME LOI POUR LES ROUES DE WARES : un ruban posé sur un nœud détaché ne bouge pas. */
import { poserLesRoues } from "./wares-ecran.mjs?v=847";
...
import { PLAFOND_HARMONISATION } from "./x1-ecran.mjs?v=847";
/* le panier du document — mêmes lecteurs que les écrans, jamais une copie */
import { currentCartLines, nextCartIndex } from "./equipement-pipeline.mjs?v=847";
import { CURRENCY_KEYS } from "../../src/build/index.mjs?v=847";
...
import { createDocWriters } from "../../src/doc/writers.mjs?v=847";
...
import { injecte, render as renderFiche } from "../../src/tools/render-fiche.mjs?v=847";
...
import { canonicalText } from "../../src/doc/canonical.mjs?v=847";
...
import { platformNow } from "../../src/doc/clock.mjs?v=847";
import { ouvrirOnglet, telecharger } from "./fichier.mjs?v=847";
...
import { ouvrirLeMagasin as monterLeMagasin, choisirUnDossier, garderDansLeTiroir,
  baseIndexedDb, dossierPossible } from "./magasin.mjs?v=847";
import { popupDeLaDestination } from "./magasin-ecran.mjs?v=847";
...
import { versionQuery } from "./version.mjs?v=847";
...
import { etatNeuf, remettreAZero } from "./etat-neuf.mjs?v=847";
...
/* ⚖️ LOT 200 — ERIC, 13/09 : *« quand je fais Build a character, je veux me
   retrouver dans le CHAPITRE 1. »*
   📏 MESURÉ AVANT : la voie atterrissait bien sur Identity, mais SEULEMENT une
   fois le popup répondu — entre l'appui et la réponse, l'écran du dessous
   restait le MENU. Le joueur pressait « Build a character » et continuait de
   regarder la page qu'il venait de quitter.
   ⭐ ET C'EST LA VOIE DE CRÉATION SEULE : `remettreAZero` sert AUSSI à ouvrir
   un personnage rangé, et celui-là revient au Menu (le rang R du magasin, lot
   195). Poser le cran dans l'état neuf aurait déplacé les deux.
   ⛔ TROUVÉ PAR L'ID, jamais par la position — la loi de `REVIEW_INDEX` juste
   au-dessus : Identity est le premier chapitre PARCE QUE c'est Identity. */
const CONCEPT_INDEX = STEPS.findIndex((step) => step.id === "concept");
...
/** 🌱 LOT 198 — LE MOT DU CRAN VOISIN, tel que la ceinture le montre, ou
 *  `null` en bout de course. Un écran qui veut dire « Next moves on to … »
 *  le REÇOIT d'ici : écrit en dur, « Skills » mentait en pile SRD (mesuré). */
function motDuCranVoisin(sens) {
  const rang = cranVoisin(sens);
  const cran = rang === state.step ? null : cransAlignes(drapeauxMontes())[rang];
  return cran ? cran.mot : null;
}

/** 🌱 LOT 198 — CE QUE LA COQUILLE SAIT DU PERSONNAGE, tendu aux crans.
 *  ⛔ IL N'Y A PLUS DE LISTE D'ÉCRANS PAR NOM ICI. Six écrans étaient nommés
 *  (`ECRANS_QUI_LISENT_LA_FICHE`) et tués ensemble dès que la dérivation était
 *  impossible — dont Abilities, l'écran qui POSE les scores que la dérivation
 *  exige : le personnage neuf ne pouvait jamais en sortir. Le cran déclare
 *  ce qu'il lit (`lit`, etapes.mjs) ; la coquille ne fait que tendre les
 *  FAITS, lus sur la donnée : le refus de `rebuild`. Un seul fait, parce
 *  qu'un seul chapitre déduit (Eric, 10/09) — le premier passage en tendait
...
/* 🌱 LOT 197 — LA DÉCLARATION DE `state` A DÉMÉNAGÉ DANS `etat-neuf.mjs`, ET
   ELLE Y EST DEVENUE LA SOURCE DE LA REMISE À ZÉRO. Lire la tête de ce
   fichier-là avant d'ajouter un champ ici : depuis ce lot, **un champ neuf est
   remis à zéro par défaut**, et le faire SURVIVRE demande de l'inscrire
   nommément dans `CHAMPS_QUI_SURVIVENT`, avec son argument. ⛔ Le sens de la
   liste s'est inversé — 📏 elle en remettait 18 sur 41, elle en remet 31.
   ⭐ ET ELLE EST SORTIE D'ICI POUR QU'UN GARDE PUISSE LA COMPTER : personne
   n'importe `shell.mjs` (SOCLE, garde F de `tests/socle.test.mjs`), donc aucun
   test ne pouvait énumérer les clefs RÉELLES de `state` tant qu'elles vivaient
   dans ce fichier. `tests/etat-neuf.test.mjs` les compte maintenant sur la
   donnée, jamais sur une liste recopiée. */
...
/** LE PERSONNAGE NEUF — un document `fh-char/1` qui ne porte que son niveau
 *  de naissance (lot 198 : un personnage naît au niveau 1, et c'est `composer`
 *  qui l'écrit — voir sa tête), fabriqué par l'écrivain PUR du bloc `doc`
 *  (`composer`, src/doc/writers.mjs), le MÊME que `doc.create` appelle. ⛔ Pas un objet littéral écrit ici : deux formes du
 ...
 *  **23 survivaient** — dont la fiche dérivée du personnage précédent, sa
 *  branche de Menu, sa méthode d'abilities, son panneau d'Inheritance déplié.
 *  ⛔ LA LISTE N'A PAS ÉTÉ RALLONGÉE, ELLE A ÉTÉ INVERSÉE : `remettreAZero`
 *  (`etat-neuf.mjs`) RECONSTRUIT l'état depuis sa déclaration et ne garde que
 *  ce que `CHAMPS_QUI_SURVIVENT` nomme. Rallonger aurait laissé le défaut
 *  intact — une liste PAR NOM ne dit jamais qu'elle est incomplète, et le champ
 *  ajouté demain aurait été oublié pareil, en silence.
 *  ⇒ **Ajouter un champ à `state` ne demande plus rien ici.** Le faire
 *  SURVIVRE, en revanche, est une décision qui s'écrit à côté de son nom.
 *
 *  ⛔ IL NE TOUCHE PAS AU DOCUMENT : c'est de l'état d'ÉCRAN, et l'appelant
 *  pose le document lui-même (un fichier ouvert, un personnage neuf). C'est
 *  `document` qui porte cet argument dans `CHAMPS_QUI_SURVIVENT`, pas une
 *  exception muette ici.
 *
 *  ⚠️ LE MINUTEUR RESTE ICI, ET IL NE PEUT PAS PARTIR : `annulerDestiny` tient
 *  une fermeture de la coquille (`destinyTimer`), pas un champ de `state`.
 *  Remettre `destinyPhase` à « porte » sans tuer le minuteur laisserait la
 *  cérémonie du personnage d'avant repeindre l'écran du personnage neuf. */
function remettreLEcranAZero() {
  annulerDestiny();
  remettreAZero(state);
}

/* ══ 📂 UN PERSONNAGE QUI VIENT D'AILLEURS DEVIENT LE PERSONNAGE COURANT ═══
   ⭐ LOT 195 — UN SEUL ÉCRIVAIN POUR DEUX PORTES. Ces quinze lignes vivaient
   dans `ouvrirUnFichier`, et le magasin venait d'en avoir besoin MOT POUR MOT :
   rouvrir une entrée de la page et ouvrir un fichier du disque, c'est le même
   atterrissage. ⛔ Deux copies auraient raté le même champ à des dates
   différentes — la leçon du 193 sur `remettreLEcranAZero`, appliquée un cran
   plus haut. *Un organe que N écrans fabriquent sera oublié.*

   🔴 POURQUOI ON REPASSE PAR LE DÉMARRAGE. `state` porte trente et un faits
   qui appartiennent au personnage COURANT (la carte tirée de Destiny, le lot de
   dés d'Abilities, le palier, le curseur…), et un rechargement de page les pose
   TOUS par construction — c'est le seul chemin dont on n'a rien à mesurer.
   On confie donc le personnage à la mémoire du navigateur, et la page
   redémarre dessus.
   ⛔ ET ON NE RECHARGE QUE SI LA MÉMOIRE A ACCEPTÉ : recharger sur un magasin
   qui refuse (navigation privée) jetterait le personnage qu'on vient d'ouvrir.

   🌱 LOT 197 — LA BRANCHE SANS RECHARGEMENT SOUFFRAIT DU MÊME DÉFAUT QUE
   `Build a character`, ET ELLE EST RÉPARÉE PAR LE MÊME ORGANE. Mesuré le
   10/09 : elle remettait à zéro les mêmes 18 champs sur 41, donc un personnage
   ouvert par-dessus un autre héritait de son écran — ⛔ et elle le faisait
   APRÈS un premier rendu, si bien que l'ancien écran était peint une fois avec
...
function poserLeDocumentOuvert(document) {
  ...
  /* 🌱 LOT 197 — AVANT LE PREMIER RENDU, ET C'EST UN CHANGEMENT D'ORDRE.
     ...
     encore là. Sur la branche qui recharge, personne ne le voyait ; sur celle
     qui ne recharge pas (magasin refusé, navigation privée), c'était l'écran
     que le joueur gardait. ⛔ Les deux nulls écrits ici à la main
     (`ouvertureRefusee`, `memoireIgnoree`) sont partis avec : l'organe les
     porte, et deux écrivains pour le même champ est ce que ce lot retire. */
  remettreLEcranAZero();
...
function applyDecisionAction(action) {
  ...
  if (action.kind === "saveAndConfirmLayerStack") {
    /* ⏳ LOT 195 — LA SÉQUENCE ATTEND, parce que ranger une entrée touche un
       dossier ou une base et que les deux répondent en différé. La RÈGLE, elle,
       n'a pas bougé d'un mot : un Save qui a refusé n'éteint pas, et la
       question reste posée. */
    sauvegarderPuisEteindre({
  ...
  /* ⭐ LE PANNEAU DE LORE (lot 82) — et il appelle `openSurface`, PAS
     `refresh` : le lore REMPLACE la scène par une page de deux cents mots. On
  ...
  /* ⛔ L'ACTION `roll` A DISPARU AU LOT 80, ET AVEC ELLE LE DERNIER JETEUR
     ...
     redessin. */
  /* ⭐ LE RETOUR — 05/09 au soir, Eric : *« le drag and drop doit se faire en
     aller-retour »*. La clef quitte la carte `assign` et RIEN D'AUTRE ne bouge :
     pas de `set`, pas de `rebuild` (le document garde sa dernière valeur —
     `derive.mjs` jette sur une valeur manquante, et c'est la porte qui compte
     les poses). Un redessin, pour que le collecteur se vide et que le dé
     réapparaisse sur son podium. */
  /* 🔁 UNE PLACE DU PODIUM — Eric, 05/09 : *« manque la possibilité de se déplacer
     d'un podium à l'autre »*. `podium` est l'ordre des pastilles (index de jets),
     hors document comme `assign`. Lâcher le jet `rollIndex` sur la pastille du jet
     `slotOf` ÉCHANGE leurs places ; si le dé venait d'un collecteur (`key`), il
     en sort aussi. Rien n'atteint le document : c'est un rangement. */
  if (action.kind === "abilityPodium") {
    const lot = state.abilityRoll;
    if (!lot || !Array.isArray(lot.rolls)) return;
    const gardes = lot.rolls.filter((r) => r.kept !== false).map((r) => r.index);
    const podium = Array.isArray(lot.podium) && lot.podium.length === gardes.length ? [...lot.podium] : gardes;
    const a = podium.indexOf(action.rollIndex), b = podium.indexOf(action.slotOf);
    if (a < 0 || b < 0) return;
    [podium[a], podium[b]] = [podium[b], podium[a]];
    const assign = { ...(lot.assign || {}) };
    if (action.key && action.key in assign) delete assign[action.key];
    state.abilityRoll = { ...lot, podium, assign };
    refresh();
    return;
  }
  if (action.kind === "unassignAbilityRoll") {
    const lot = state.abilityRoll;
    if (!lot || !lot.assign || !(action.key in lot.assign)) return;
    const assign = { ...lot.assign };
    delete assign[action.key];
    state.abilityRoll = { ...lot, assign };
    refresh();
    return;
  }
  ...
  /* ══ LOT 214 — LES SECTIONS DU SAC ══════════════════════════════════════
     ⚖️ Eric, 18/09 : *« le bouton + crée, le bouton - supprime »* · *« possibilité
     de supprimer une section si elle est vide »*.
     ⭐ TROIS VERBES ET PAS UN DE PLUS : créer, renommer, supprimer. Le PLACEMENT
     d'un objet dans une section n'en est pas un — c'est `placerGearLine`, qui pose
     déjà `gear[N].boite` sur l'écran R. Un second écrivain pour la même
     appartenance ferait diverger le sac et le dressing. */
  if (action.kind === "ajouterSection") {
    const index = nextSectionIndex(state.document);
    /* ⛔ `Storage N`, PAS `Section N` — Eric, 19/09 : *« change `section` par : storage
       1, storage 2 »* · *« pas backpack storage, juste storage »*. ⭐ Et le nom se
       demande à l'organe qui le sait (`nomDeSectionParDefaut`) : une seconde écriture
       ici aurait diverge de celle du socle au premier renommage de la règle. */
    state.document = verbs.set({ document: state.document,
      path: `backpack.sections[${index}].name`, value: nomDeSectionParDefaut(index) }).document;
    /* ⚖️ ET LE `+` DORÉ ÉCRIT LE GENRE — Eric, 2026-09-19 : *« outside your gear (horse,
       chest, house) »* · 20/09 : *« je veux mon doré pour les other »*.
       🔴 C'ÉTAIT LE TROU : les deux `+` créaient la MÊME chose. Le liseré doré existait, le
       genre existait à l'écran, et rien ne l'ÉCRIVAIT — donc il mourait au premier
       repeint, et un objet rangé « hors du sac » pèsait quand même. ⛔ Un genre qui ne
       persiste pas n'est pas un genre, c'est une couleur.
       ⭐ UN SCALAIRE, la même forme que le rang — *« set n'accepte qu'un scalaire »*. */
    if (action.dehors === true) {
      state.document = verbs.set({ document: state.document,
  ...
  if (action.kind === "renommerSection") {
    const mot = String(action.nom ?? "").slice(0, 22).trim();
    /* ⛔ UN NOM VIDE NE S'ÉCRIT PAS : une section sans nom serait un cran muet sur
       la roue, et le joueur ne saurait plus où il est. 22 signes, la cote du cran
       sur deux étages — au-delà le mot serait tronqué à l'écran sans le dire. */
    if (!mot) return;
    state.document = verbs.set({ document: state.document,
      path: `backpack.sections[${action.index}].name`, value: mot }).document;
    rebuild(); refresh(); return;
  }
  /* ⚖️ VIDER UNE SECTION SANS LA RETIRER — Eric, 2026-09-19 : *« si moins de 5 items
     dans le tambour, n'efface pas la tuile, juste nomme-la blank, quand on efface »*.
     ⭐ LA PLACE SURVIT À SON NOM, et c'est tout le sens : sous cinq crans la roue cesse
     de boucler et montre des trous aux deux bouts. Un tiroir vidé qui reste en place
     garde la roue pleine — et il dit vrai : la place existe, elle ne contient rien.
     ⛔ C'EST LE SEUL POINT QUI ÉCRIT UN NOM VIDE. `renommerSection` les refuse, et il a
     raison : un cran muet qu'on aurait TAPÉ serait une faute de frappe. Vidé par le
     `×`, c'est une intention — deux gestes, deux verbes. */
  if (action.kind === "viderSection") {
    state.document = verbs.set({ document: state.document,
      path: `backpack.sections[${action.index}].name`, value: "" }).document;
    rebuild(); refresh(); return;
  }
  if (action.kind === "supprimerSection") {
    /* ⛔ SEULEMENT SI ELLE EST VIDE — Eric, 18/09. Toute autre règle demanderait une
       politique de migration (vers où vont les douze jetons ? et s'il n'y a de place
       nulle part ?), et chacune peut échouer au pire moment. *« Vide-la d'abord »*
       ne perd rien et se comprend sans être écrit. */
    const boite = boiteDeSection(action.index);
    const occupee = currentGearLines(state.document)
      .some((l) => (l.location || "backpack") === "backpack" && l.boite === boite);
    if (occupee) {
      state.popup = { titre: "Section", role: "gendarme",
        texte: "This section still holds items. Empty it before deleting it.", actions: null };
      refresh(); return;
    }
    state.document = verbs.clear({ document: state.document,
      path: `backpack.sections[${action.index}].name`, kind: "choice" }).document;
    rebuild(); refresh(); return;
  }
  ...
  if (action.kind === "placerGearLine") {
    let document = state.document;
    /* ⚖️ LE LIEU VIENT DE LA LOI, ⛔ IL NE SE REFAIT PAS ICI — `lieuDeLaBoite`
       (`equipment-step.mjs`). Cette décision vivait dans ce fichier, qui n'exporte
       RIEN : aucun garde ne pouvait l'interroger, et une faute y a vécu jusqu'au
       19/09 au soir — le test portait sur la FORME de la clef (`/^s\d+$/`), or celle
       du party bag est un MOT, donc un objet déposé dans le sac commun ressortait
  ...
  /* ══ DÉPLACER UNE SECTION DANS LA ROUE — Eric, 2026-09-19 au soir ═════════════
     ⚖️ *« edit mode comprenant le déplacement des storage »*, et pour le sac du groupe :
     *« on peut changer sa position, mais pas l'effacer ni la renommer »* — donc l'ordre
     le porte lui aussi.
     ⭐ IL RENUMÉROTE TOUT, comme `rangerSection` le fait des places. ⛔ Écrire le seul
     rang qui bouge laisserait un ordre à trous, où deux sections peuvent porter le même
     nombre : le tri redeviendrait dépendant de l'ordre d'arrivée, et l'écran bougerait
     sans qu'on sache pourquoi.
     📏 LE CHEMIN EST MESURÉ CONTRE LE MOTEUR (protocole P1, 19/09) : scalaire accepté,
     0 violation, `unconsumed`, `clear` propre. ⛔ ET LA LISTE A ÉTÉ REFUSÉE — *« set
     n'accepte qu'un scalaire, une structure serait une règle déguisée »*. D'où N
     écritures et pas un tableau, et d'où `cheminDuRang`, qui sait que le party bag n'a
     pas d'index. */
  if (action.kind === "ordonnerSections") {
    const ordre = Array.isArray(action.ordre) ? action.ordre : [];
    if (!ordre.length) { refresh(); return; }
    let document = state.document;
    ordre.forEach((clef, rang) => {
      document = verbs.set({ document, path: cheminDuRang(clef), value: rang }).document;
    });
    state.document = document;
    rebuild();
    refresh();
  ...
  if (action.kind === "moveGearLine") {
    ...
    /* 🔴 ET LA BOÎTE SE VIDE, parce qu'elle appartient au LIEU qu'on quitte —
       ...
       `boite: "tete1"`. Personne ne le voyait tant que seul R lisait ce champ ; le
       jour où le sac le lit, c'est une section fantôme.
...
function skillsCtx() {
  return {
    /* 🌱 LOT 198 — le document, pour que l'écran NOMME ce qui manque à la
       fiche (`motDuManque`, ecran-mort.mjs) à la place du pool quand elle
       n'existe pas — Skills vit sans fiche, il ne ment pas et ne meurt pas. */
    document: state.document,
...
const CATALOGUES = {
  ...
  destiny: {
    ...
    /* 🔤 LE RAIL DES 22 PORTE LE CHIFFRE ET LE NOM — Eric, 2026-09-03, après
       cinq jeux de pictogrammes jugés et refusés : *« les symboles sont super
       moches »*, puis *« le numéro en T2, le nom en T1 »*.
       ⛔ CE QUI EST RETIRÉ : `railImage`, et avec lui les 22 SVG. Ils vivent
       toujours dans le vault et dans l'historique — mais un asset que l'écran
       n'appelle plus est une dette, pas une réserve.
       ⭐ ET LE NOM RÈGLE LES TROIS REPROCHES D'UN COUP : plus de dessin à juger,
       plus de couleur de trait à accorder au jour et à la nuit (du texte prend
       l'encre du thème par construction), et le scrollspy dit enfin QUOI et
       plus seulement OÙ. */
    railEtiquette: (id) => arcanaNumeral(state.engine.layers.verbs.query, id),
    railEtroit: true,
    titreDansLaFiche: true,
    /* le corps rend sa propre dalle : la rangée reste nue (un seul voile) */
    corpsEstUneDalle: true,
    ...
    retourInterne: true,
    /* ⛔ et pas de `Done` : c'est `Choose` qui ouvre, et le `Next` de SB2 qui acte */
    sansDone: true,
    /* ⚖️ `Cancel`, pas `Back` : on ABANDONNE la lecture des 22, on ne recule pas
       d'un palier — et rien n'est effacé, puisque rien n'a été posé. */
    motDuRetour: "Cancel"
  }
};
...
function catalogueCtx(cfg) {
  return {
    ...
    corpsEstUneDalle: Boolean(cfg.corpsEstUneDalle)
  };
}
...
function renderStepContent() {
  ...
  /* ══ L'ÉCRAN QUI NE PEUT PAS SE DESSINER — et qui le DIT ═══════════════
     🌱 LOT 198 — LE CRAN DÉCLARE CE QU'IL LIT, LA COQUILLE TEND LES FAITS.
     ...
     Sheet ? »* — les chapitres travaillent sur les choix ; UN SEUL chapitre
     déduit : Sheet. Un seul cran déclare donc une lecture (`lit`, etapes.mjs) :
     `review`. Tout le reste vit du document et du carnet — Abilities en tête,
     l'écran qui POSE les scores : le tuer parce que les scores manquaient
     était le cercle qui rendait tout personnage neuf infinissable (mesuré sur
     v621). Skills et Equipment vivent sans fiche et sans classe, et NOMMENT là
     où un chiffre déduit devrait s'afficher (voir leurs têtes).
     📏 L'ANCIENNE LISTE PAR NOM (background, abilities, destiny, skills,
     equipment, review) était fausse deux fois — Destiny ne lit pas `resolved`,
     Abilities le lit sans en dépendre — et la coquille ne pouvait pas le voir.
     ⭐ LES CATALOGUES N'ONT PLUS BESOIN D'UNE CLAUSE : aucun cran-catalogue ne
     déclare de lecture, donc Class — l'endroit où l'on répare l'état où l'on
     est — reste ouvert par construction, sans `!catalogueCourant()`.
     📌 ET LE MOT NOMME LE MANQUE ET LA SORTIE, depuis la donnée
     (`motDeLEcranMort` : la classe, les scores, le niveau, chacun avec son
     cran) — la règle du refus qui nomme, §5 du canon. */
  ...
  /* ⚠️ PAS AVANT LE PERSONNAGE : tant que le boot n'a pas posé de document, il
     n'y a aucun fait à tendre — les branches « Loading the engine… » plus bas
  ...
  /* LOT 42 — Class et Species suivent EXACTEMENT le patron Compétences
    ...
    card.append(renderAbilitiesStep({
      ...
      /* 🌱 LOT 198 — le cran que `Next` atteindra, lu sur la ceinture (jamais écrit) */
      suivant: motDuCranVoisin(1)
    }, applyDecisionAction));
  ...
  if (guide && !card.querySelector(".tuto-general, .tuto-specifique")) {
    /* ⚠️ ÉLARGI LE 2026-09-03, ET LA RÈGLE ÉTAIT DÉJÀ ÉCRITE À CÔTÉ : *« là où
       les pages ALTERNENT — un rail, une fiche visible à la fois — chacune
       porte la sienne »*. Le catalogue de Destiny est exactement ce cas, mais
       il n'emploie pas `.fiche-dalle` : ses fiches sont des dalles aimantées.
       Résultat mesuré : UNE pastille pour vingt-deux fiches, et Eric l'a vue
       avant moi — *« il y a toujours ? et livre »*.
       ⭐ Le second motif nomme le FAIT (une dalle dans un cran d'aimantation),
       pas un genre : le prochain catalogue à rail en héritera sans qu'on y
       revienne. */
    const fiches = card.querySelectorAll(".fiche-dalle, [data-snap] > [data-objet='dalle']");
...
function renderParcoursGuide(cfg, ctx) {
  ...
  return renderGuideSpecifique({
    ...
    bilanLabel: cfg.bilanLabel ? (item) => cfg.bilanLabel(item.path, ctx) : null,
...
const GUIDES = {
  ...
  /* ⚠️ CES DEUX TUTORIELS ONT ÉTÉ PÉRIMÉS PAR UN RENOMMAGE (2026-09-05) : ils
     disaient encore « I changed my mind » quand le bouton s'appelait déjà
     `Cancel` depuis le lot du même jour. Le lot qui a renommé n'a touché que
     `parcours-ecrans.mjs` — le texte qui NOMME le bouton vivait ici.
     ➡️ La règle : `ui/builder/NORMES.md` § 6 pré bis, « l'aiguilleur et le
     tutoriel disent la même étape ». ⛔ Elle n'est pas recopiée ici. */
  species: {
    titre: "Species",
    texte:
  ...
  class: {
    ...
    texte:
      ...
      "What it asks of you is listed as openable lines: open each one and mark it Done.\n" +
      "Cancel releases the class and gives the rail back."
  },
  /* 🔴 RÉÉCRIT LE 2026-09-05, POUR DEUX RAISONS QUI N'EN FONT QU'UNE.
     ⑴ Il citait `INFO`, un panneau qui n'existe plus : le livre mène désormais
     au chapitre `ability-scores` de FH WEB. Une consigne qui nomme un organe
     retiré n'est pas seulement inutile — elle envoie le joueur chercher.
     ⑵ Eric, le même jour : *« le `?` est un tutoriel plus verbeux, plus
     agréable à lire, que l'aiguilleur »*. L'aiguilleur de cet écran tient en
     une phrase qui POINTE (« Pick one of the methods below to begin ») ; le
     tutoriel a le droit de RESPIRER, et c'est ce qui les distingue.
     ⑶ 06/09 — IL RECUEILLE LES DEUX RÈGLES DE `FREE`. Sa page a perdu son
     aiguilleur (le budget du 4 × 4 ne l'admet pas, NORMES §7.12), et cet
     aiguilleur était le SEUL endroit du produit qui écrive *« take any value, as
...
function pressDone() {
  ...
  if (gate.next === "bilan") {
    ...
    state.palier = 1;
    /* 🌱 LATE BLOOMER — LE DÉCLENCHEUR PART AU PERSONNAGE ICI, ET NULLE PART
       AILLEURS (lot 169). C'est le même instant que la signature juste dessous,
       et pour la même raison : le bilan EST le moment où les six scores sont
       arrêtés, donc le moment où « le plancher haut a dû intervenir » cesse
       d'être un état d'écran pour devenir un fait du personnage.
       ⛔ ET IL SE LÈVE AVEC LA SIGNATURE : `leverLaSignatureDAbilities` l'efface,
       de sorte qu'un lot jeté ou une méthode changée ne laissent jamais derrière
       eux un trait que plus aucun dé ne justifie. Un seul écrivain, deux gestes
       symétriques. */
    poserLeTraitTardif();
...
function leverLaSignatureDAbilities() {
  /* 🌱 LE TRAIT PART AVEC LA SIGNATURE (lot 169), et il part D'ABORD — l'effacer
     est légitime même sur une étape que rien n'a signée (un lot jeté avant
     `Done`), alors que le `revoke` ci-dessous, lui, n'a rien à faire dans ce
     cas-là. Les mettre dans l'autre ordre laisserait un trait derrière un
     `return`, sans un mot. */
  effacerLeTraitTardif();
...
/* ══ 🌱 LATE BLOOMER — LES DEUX MOITIÉS DU MÊME ÉCRIVAIN (lot 169) ═══════
   🔴 CE QUE CE COUPLE RÉPARE, ET C'EST LE BLOCAGE QUI COMMANDAIT LE LOT :
   `ajuste: "haut"` — le drapeau que le moteur des dés lève quand le plancher
   haut a dû rattraper un tirage — vit dans `state.abilityRoll`, un état
   d'ÉCRAN. Le personnage sauvegardé ne garde que ses six scores. Un fichier
   rouvert n'avait donc AUCUNE mémoire du déclencheur, et le moteur ne pouvait
   pas appliquer l'effet du trait.

   ⭐ CE QUI EST ÉCRIT EST UN FAIT, PAS UN EFFET : « ce personnage porte
   `late-bloomer` ». Les deux points libres et le droit d'acheter l'expertise au
   niveau 1 sont chiffrés dans la couche et appliqués par le module du pool —
   la coquille ne calcule rien, n'offre rien, et ne connaît pas le prix.

   ⛔ UN SEUL ÉCRIVAIN POUR LES DEUX GESTES. Poser ailleurs (au tirage, par
   exemple) ferait vivre un trait sur un personnage dont les scores peuvent
   encore bouger ; effacer ailleurs laisserait un trait qu'aucun dé ne justifie
   plus. Les deux moitiés sont ici, à côté l'une de l'autre, pour qu'on ne
   puisse pas en déplacer une sans voir l'autre. */
function poserLeTraitTardif() {
  if (!state.document || !state.engine) return;
  /* La condition est LUE au lot de dés, jamais recalculée : `lotRattrape` est
     la seule lecture du drapeau, et elle vit avec l'écran qui le produit. */
  if (!lotRattrape(state.abilityRoll)) return;
  state.document = state.engine.build.verbs
    .set({ document: state.document, path: CHEMIN_TRAIT_TARDIF, value: true }).document;
  rebuild();
}

function effacerLeTraitTardif() {
  if (!state.document || !state.engine) return;
  state.document = state.engine.build.verbs
    .clear({ document: state.document, path: CHEMIN_TRAIT_TARDIF, kind: "choice" }).document;
  rebuild();
}
...
/** 🗣️ L'HÔTE PEUT DÉCLARER SA SORTIE — 2026-09-05, pour les deux scènes de
 *  B1 (Abilities). Trois faits lisibles sur `[data-sortie-ici]`, jamais un id :
 *  `data-sortie-mot` (le mot du retour), `data-sortie-sans-done` (rien à
 *  valider ici), `data-sortie-verbe="abilityClear"` (le retour EFFACE le lot au
 *  lieu de reculer d'un palier). ⛔ Un seul producteur, toujours : c'est cette
 *  fonction qui fabrique la paire — l'hôte ne fait que dire ce qu'il attend,
 *  exactement comme la table des catalogues le fait pour les fiches. */
function renderSortieEtape(hote) {
  const decl = hote && hote.dataset ? hote.dataset : {};
  ...
  /* ══ 🚪 …ET JAMAIS DANS UNE DALLE D'ITEM — LOT 194 ════════════════════════
     Eric, 2026-09-10, sur le fil SRD : *« B1 : […] pas de bouton Back /
     Done. »*

     📏 MESURÉ DANS LA PAGE LE 10/09 (375 × 812, pile SRD) sur le personnage
     d'exemple, dont l'étape Species est SIGNÉE : ouvrir la porte `Species
     skill` rendait une dalle d'item **sans aucune sortie** — l'hôte
     `[data-sortie-ici]` ne recevait que le livre, éteint. Ni `Cancel` ni
     `Done` : le cran le plus intérieur du parcours n'avait plus de porte, dans
     les deux sens. Sur un personnage NEUF la paire est là ; c'est la signature
     de la racine qui la faisait disparaître.
     ⚠️ ET CE N'EST PAS UN CAS TORDU : une étape signée rouvre ses portes dès
     qu'un item nouveau apparaît (une couche qu'on éteint, un plan qui grandit)
     ou qu'un verrou en rougit une — c'est exactement le fil qu'Eric refaisait.

     ⭐ LA CLAUSE D'AU-DESSUS VISE L'ÉCRAN D'ÉTAPE, PAS SON SOUS-ÉCRAN. Elle
     naît du *« Done dans le vide après validation de Identity »* (26/08) :
     l'étape est validée, il n'y a plus rien à y valider. Un ITEM, lui, a
     toujours quelque chose à valider — c'est sa seule raison d'être — et sa
     paire est le seul chemin de retour, la ceinture ne remontant qu'entre
     ÉTAPES. La même exception est déjà écrite deux clauses plus bas pour le
     drapeau `fiche`, avec le même argument et la même mesure (*« Sans cette
     exception, l'item s'ouvrait sans aucune porte »*, 19/08) : elle manquait
     ici, et c'est le même défaut qui est revenu par l'autre porte. */
  if (estConfirme(state.document, STEPS[state.step].id) && !decl.sortieVerbe && !state.parcoursItem) return null;
  ...
  /* 🔴 LE MOT SUIT LE GESTE, ET UN ÉCRAN PEUT LE DÉCLARER — Eric, 2026-09-03 :
     *« cancel / choose »* en B2, *« i changed my mind / next »* en SB2.
     ⭐ `Cancel` N'EST PAS UN SYNONYME DE `Back` DANS CE DÉPÔT : il ABANDONNE ce
     qu'on faisait sans rien effacer (Eric, 2026-08-19 : *« quitter un item n'est
     pas reculer d'un cran »*). Quitter le catalogue, c'est exactement ça — on
  ...
  const motDuRetour = (state.parcoursItem || effaceAuRetour) ? "Cancel"
    : decl.sortieMot || (cfgRetour && cfgRetour.motDuRetour) || "Back";
  ...
  /* 🔴 LA COULEUR SUIT L'ACTE, ET NORMES LE DIT DEPUIS TOUJOURS : *« les
     ...
     ⚖️ `Back` recule sans rien défaire → le bleu du recul (`--info`).
     `Cancel` ABANDONNE ce qu'on faisait → le rouge de ce qui coûte
  ...
  if (state.parcoursItem) {
    ...
    /* ══ 🔴 LOT 194 — ET ON NE SIGNE PAS NON PLUS UN ITEM QUI N'A PAS RÉPONDU ═
       📏 MESURÉ DANS LA PAGE LE 10/09, en vérifiant le lot : ouvrir `Species
       skill` sans rien poser et presser `Done` SIGNAIT l'item (`build.confirmed`
       le porte), le voyant verdissait, et l'étape annonçait *« This step is
       settled »* — sur une compétence jamais choisie. Le défaut est ANCIEN et
       général : `done.disabled` ne lisait que `gate.ready`, qui compte le
       PALIER, pas le contenu de l'item.

       ⚠️ ET IL FALLAIT LE VOIR MAINTENANT : `background.boost` en était protégé
       PAR ACCIDENT — son verrou de total partait dès zéro (c'est le rouge
       qu'Eric fait tomber ce lot), et c'est lui qui désarmait le `Done`. Retirer
       le verrou sans poser le compte aurait échangé un faux rouge contre un faux
       « settled » : un arrière-plan signé à 0 point sur 3.
       ⭐ LE COMPTE FAIT LE TRAVAIL QUE LE VERROU FAISAIT PAR HASARD, et c'est la
       phrase exacte du lot : « un compte qui n'a pas commencé n'est pas un
       compte faux » — il retient la porte, il n'accuse pas.
       ⛔ SEULEMENT QUAND L'ITEM A UN PLAN : un sous-écran que le carnet ne
       publie pas (un guide emboîté) garde son `Done`, sinon on poserait un
       bouton mort sur un écran qui n'a rien à compter.
       📌 `Cancel` reste armé — Eric, 29/08 : *« ainsi que la possibilité de
       revenir en arrière »*. */
    if (plan && !repondu && done && !done.dataset.verrou) {
      done.disabled = true;
...
function poserLaSortie(contenu, sortie) {
  ...
  /* 🔴 UNE SORTIE PAR HÔTE — Eric, 2026-09-03, en regardant le catalogue sur son
     ...
     sortie et cette fonction la déposait dans le PREMIER hôte trouvé. Vingt-deux
     fiches déclaraient `data-sortie-ici`, une seule recevait sa paire — et c'est
     invisible tant qu'on ne fait pas défiler.
     ⭐ LA COQUILLE RESTE LE SEUL PRODUCTEUR (garde 17) : elle en fabrique
     autant qu'il y a d'hôtes, au lieu d'en cloner une — un clone perdrait ses
     écouteurs, et un bouton muet est pire qu'un bouton absent. */
  const hotes = [...contenu.querySelectorAll("[data-sortie-ici]")];
  if (hotes.length === 0) return noeuds;
  if (hotes.length > 1) {
    /* ⛔ AUCUNE RETOMBÉE SUR LA CARTE ICI, ET C'EST LA CONDITION DE TOUT LE
       CORRECTIF D'EN DESSOUS : quand les pages ALTERNENT, chaque fiche porte SA
...
function poserLesBornes(racine) {
  for (const borne of racine.querySelectorAll(BORNES)) {
    ...
    /* 🔴 « UNE CELLULE CONTENANT AU MOINS UN BOUTON » — Eric, 2026-09-04.
       ⭐ LE DUO ENCADRE DES GESTES, IL N'EN EST PAS UN. Une rangée qui ne
       porterait que le livre et le `?` serait deux bornes autour de rien : la
       colonne du milieu resterait vide et les deux ronds se liraient comme les
       boutons de l'écran, ce qu'ils ne sont pas. §7 le dit de la pastille —
       *« un `?` qui n'ouvre rien apprend à ne plus le regarder »* — et c'est la
       même famille : un organe qui prend la place d'un geste sans en être un.
       ⛔ On cherche donc un MAJEUR, pas un bouton : le livre et le `?` en sont
       eux-mêmes, et se compter soi-même n'a jamais rien prouvé. */
    if (!rangee.querySelector("button:not(.fiche-livre):not(.tuto-point)")) continue;
...
function refresh() {
  ...
  /* ⭐ ET LE RUBAN DE DALLES SE POSE ICI, POUR LA MÊME RAISON, À LA MÊME LIGNE : c'est
     l'instant où le contenu entre dans le document, et une position de défilement ne
     s'écrit pas sur un nœud détaché. 🔴 L'étape, elle, ne peut PAS le faire — mesuré :
     son `peindre()` tourne sur un arbre détaché (`isConnected: false`, `clientWidth: 0`),
     et l'écriture y tombait dans le vide sans rien dire.
     📏 Ce qu'on répare, relevé dans l'application toutes les 40 ms : 910 → **0** → 585.
     La roue attendait une image pour se replacer, donc entre le repeint et l'image
