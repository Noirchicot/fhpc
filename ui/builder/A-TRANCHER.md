...
## C40 — Un popup qui fait CHOISIR porte deux boutons du même vert { #c40 }

**Question : quand un popup pose un choix entre deux voies également légitimes, les deux boutons restent-ils verts — ou l'un des deux change de teinte, et lequel ?**

- 📏 **LE FAIT, MESURÉ AU NAVIGATEUR LE 21/09** *(512 × 764, les deux thèmes, les vrais boutons)* : les deux boutons de « SRD or Fate's Hand? » portent **le même** `--bouton-fond: var(--positive)` — #326d4b le jour, #469d6a la nuit. ⛔ **Aucun des deux n'a choisi cette teinte** : elle vient du défaut de la famille, `.fiche-action { --bouton-fond: var(--positive) }` *(shell.css)*. Le popup ne peint que le cas `defait` *(`--critical`, shell.css `.popup-action[data-defait="true"]`)* ; tout ce qui n'est pas « défaire » retombe donc sur le vert.
- 🔴 **ET CE N'EST PAS UN CAS ISOLÉ — C'EST UNE FORME.** Sur les **neuf** producteurs de popup à actions du dépôt, **deux posent un CHOIX à deux voies** et rendent donc deux boutons verts côte à côte : `SRD` / `Fate's Hand` *(universe-step.mjs)* et `Keep them here` / `Choose a folder` *(magasin-ecran.mjs)*. Les trois qui **défont** (`Remove` ×2, `Throw away`) portent bien le rouge ; les quatre autres lisent ou rangent.
- ⚖️ **CE QUE LA LOI DIT AUJOURD'HUI** *(NORMES, `bouton-trois-verbes`, 26/08)* : *« `Back`/`Next` NAVIGUENT (bleu), `Done` VALIDE (vert), `Cancel` DÉFAIT (rouge + popup) »*. **Choisir entre deux jeux de règles n'est aucun des trois** — ce n'est ni un déplacement du regard, ni la signature de ce qui est là, ni une destruction. ⭐ Et **deux boutons du même vert n'opposent rien** : la couleur ne distingue plus les deux voies, seul le mot le fait.
- ⛔ **LE LOT 244 N'A PAS TRANCHÉ, ET C'EST VOULU.** Eric n'a parlé que de **lisibilité** *(l'encre, réparée : 1,14:1 → 9,87:1)*. ⭐ C'est le traitement déjà appliqué à `§ C29` — *« un geste qui navigue et qui est vert n'entre dans aucune des trois familles »*, laissé à Eric — et à `§ C24` : **une norme neuve ne se câble pas en repeignant au hasard ce qu'elle condamne.**
- ➡️ Quatre lectures : *(a)* les deux restent verts — un choix EST une validation, et le mot suffit à les distinguer ; *(b)* une **quatrième famille**, CHOISIR, avec sa teinte, à nommer dans les trois verbes ; *(c)* la voie « par défaut » reste verte et l'autre passe au **bleu** *(elle ne détruit rien, elle mène ailleurs)* ; *(d)* les deux passent au bleu, et le vert est réservé au `Done` qui signe. ⚠️ Quelle que soit la réponse, elle se câble **au défaut de la famille** *(le `--bouton-fond` du popup)*, ⛔ jamais popup par popup.
